...
public abstract class Article implements Comparable<Article> {
    ...
    public boolean hasValidLastDigit(int barcode) {
        ...
        int[] barcodeNumbers = new int[7];
        for (int i = 6; i >= 0; i--) {
            barcodeNumbers[i] = barcode % 10;
            barcode /= 10;
        }
        int queersum = 0;
        for (int i = 0; i < 7; i++) {
            if(i % 2 == 1) {
                queersum += barcodeNumbers[i] * 3;
                continue;
            }
            queersum += barcodeNumbers[i];
        }
        if (10 - queersum % 10 == lastDigit) {
            return true;
        }
        return false;
    }
